...
export default function RegisterForm({ onSwitchTab, navigate, onLoginSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data?.email)    setApiError(data.email[0]);
      else if (data?.password) setApiError(data.password[0]);
    ...
    <form className="auth-form" onSubmit={handleSubmit} noValidate>
      ...
      <div className={`auth-form__field ${errors.password ? "auth-form__field--error" : ""}`}>
        ...
        <div className="auth-form__input-wrap">
          <input className="auth-form__input" type={showPassword ? "text" : "password"} placeholder="Minimum 8 caractères" value={form.password} onChange={set("password")} autoComplete="new-password" />
      ...
      <div className={errors.terms ? "auth-form__field--error" : ""}>
        <label className="auth-form__checkbox">
          ...
          <span className="auth-form__checkbox-label">
            J'accepte les <button type="button" className="auth-form__switch-link">conditions d'utilisation</button>
          </span>
        </label>
      ...
      <p className="auth-form__switch">
        Déjà un compte ?{" "}
        <button type="button" className="auth-form__switch-link" onClick={() => onSwitchTab("login")}>Se connecter</button>
      </p>
    </form>
  );
}
